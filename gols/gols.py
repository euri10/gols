...
def cli(ctx, debug):
    ...
    ctx.obj = Config(config, conf_dir_fit)
    # do we need output ?


class Config(object):
    def __init__(self, config, conf_dir_fit):
        self.username = config['username']
        self.password = config['password']
        self.conf_dir_fit = conf_dir_fit
...
def upload(ctx, directory_fit, notification, move):
    username = ctx.username
    password = ctx.password
    conf_dir_fit = ctx.conf_dir_fit
